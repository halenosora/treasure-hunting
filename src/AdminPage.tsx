    ...
    html: `
      <div style="position:relative;width:${s}px;height:${s}px;">
        ...
        <div style="
          ...
          box-shadow:0 0 ${selected ? 20 : 10}px ${color}88, 0 0 ${selected ? 40 : 20}px ${color}44;
          display:flex;align-items:center;justify-content:center;
        ">
          <div style="
            width:${s * 0.4}px;height:${s * 0.4}px;
            border-radius:50%;
            background:white;
            opacity:0.9;
...
const EMPTY_CHEST: Chest = {
  name: '', type: '地域クーポン', lat: 0, lng: 0,
  gold_amount: 100, is_active: true,
  shop_name: '', shop_photo: '', shop_url: '', shop_tel: '', description: '',
};
...
export default function AdminPage({ onClose }: { onClose: () => void }) {
  ...
  function handleMapClick(lat: number, lng: number) {
    if (editing && selected) {
      ...
      setSelected({ ...EMPTY_CHEST, lat, lng });
      setEditing(true);
    }
  }
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      gold_amount: selected.gold_amount, is_active: selected.is_active,
      shop_name: selected.shop_name, shop_photo: selected.shop_photo,
      shop_url: selected.shop_url, shop_tel: selected.shop_tel,
    ...
    if (error) showToast('❌ 保存失敗: ' + error.message);
      ...
      showToast(selected.id ? '✅ 更新しました' : '✅ 宝箱を追加しました');
      setEditing(false); setSelected(null); fetchChests();
    }
    setSaving(false);
  }
  ...
  const inp: React.CSSProperties = {
    ...
    borderRadius: 8, color: '#e8d5a3', fontSize: 13, boxSizing: 'border-box',
  };
          ...
          <div style={{ width: 300, background: '#150d2a', borderLeft: '1px solid rgba(255,215,0,0.1)', overflowY: 'auto', boxSizing: 'border-box', flexShrink: 0 }}>
            ...
            {selected && !editing && (
              <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{ fontSize: 32, textAlign: 'center' }}>
                  <div style={{ width: 48, height: 48, borderRadius: '50%', background: TYPE_CONFIG[selected.type as ChestType]?.color, margin: '0 auto', boxShadow: `0 0 20px ${TYPE_CONFIG[selected.type as ChestType]?.color}` }} />
                </div>
            ...
            {selected && editing && (
              <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <label style={lbl}>宝箱名 *
                  <input value={selected.name} onChange={e => setSelected({ ...selected, name: e.target.value })} placeholder="例：旅のお供" style={inp} />
                </label>
                ...
                <label style={lbl}>店舗名
                  <input value={selected.shop_name} onChange={e => setSelected({ ...selected, shop_name: e.target.value })} placeholder="例：横浜観光センター" style={inp} />
                </label>

                <label style={lbl}>店舗写真URL
                  <input value={selected.shop_photo} onChange={e => setSelected({ ...selected, shop_photo: e.target.value })} placeholder="https://..." style={inp} />
                </label>

                <label style={lbl}>公式サイトURL
                  <input value={selected.shop_url} onChange={e => setSelected({ ...selected, shop_url: e.target.value })} placeholder="https://..." style={inp} />
                </label>

                <label style={lbl}>電話番号
                  <input value={selected.shop_tel} onChange={e => setSelected({ ...selected, shop_tel: e.target.value })} placeholder="045-000-0000" style={inp} />
                </label>

                <label style={lbl}>説明文
                  <textarea value={selected.description} onChange={e => setSelected({ ...selected, description: e.target.value })} placeholder="宝箱の説明やヒントなど" rows={3} style={{ ...inp, resize: 'vertical' }} />
                </label>

                <label style={lbl}>ゴール量
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={handleSave} disabled={saving}
                    style={{ flex: 2, padding: '10px', background: '#c8a217', border: 'none', borderRadius: 8, color: '#1a0a2e', fontWeight: 700, cursor: 'pointer' }}>
                    {saving ? '保存中...' : selected.id ? '💾 更新' : '✨ 追加'}
                  </button>
                  <button onClick={() => { setEditing(false); if (!selected.id) setSelected(null); }}
                    style={{ flex: 1, padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, color: '#e8d5a3', cursor: 'pointer' }}>
                    キャンセル
                  </button>
                </div>
              </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
          {chests.length === 0 ? (
            ...
            <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 14, background: 'rgba(255,255,255,0.04)', borderRadius: 12, marginBottom: 10, borderLeft: `4px solid ${TYPE_CONFIG[c.type as ChestType]?.color ?? '#888'}` }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: TYPE_CONFIG[c.type as ChestType]?.color, boxShadow: `0 0 10px ${TYPE_CONFIG[c.type as ChestType]?.color}`, flexShrink: 0 }} />
              ...
              <button onClick={() => { setSelected(c); setEditing(true); setTab('map'); }}
                style={{ width: 32, height: 32, background: 'rgba(156,111,204,0.2)', border: 'none', borderRadius: 8, color: '#9c6fcc', cursor: 'pointer', flexShrink: 0 }}>✏️</button>
              <button onClick={() => handleDelete(c.id!)}
                style={{ width: 32, height: 32, background: 'rgba(204,68,68,0.2)', border: 'none', borderRadius: 8, color: '#cc4444', cursor: 'pointer', flexShrink: 0 }}>🗑️</button>
            </div>
          ))}
        </div>
      )}
